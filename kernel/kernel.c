...
void kernel_main() {
    ...
    //test_large_kfree(); // test that kfree can handle large allocations (like our ELF segment) without crashing
...
void test_large_kfree(void) {
    serial_write_string("test_large_kfree: begin\n");

    void* p = kmalloc(0x7A8);   // same size as your ELF segment
    serial_write_string("test_large_kfree: ptr = ");
    serial_write_hex32((uintptr_t)p);
    serial_write_string("\n");

    kfree(p);

    serial_write_string("test_large_kfree: survived kfree\n");
}
